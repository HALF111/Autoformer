...
def main():
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            test_data_2, test_loader_2 = exp._get_data_at_test_time(flag='test')

            with open("./file_1.txt", "a") as f1:
                for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                    print(batch_y[:, -1:, ])
                    print(batch_y[:, -1:, ].shape)
                    f1.write(str(batch_y[:, -1:, ]))

            with open("./file_2.txt", "a") as f2:
                for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader_2):
                    print(batch_y[:, -1:, ])
                    print(batch_y[:, -1:, ].shape)
                    f2.write(str(batch_y[:, -1:, ]))
