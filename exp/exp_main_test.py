...
class Exp_Main_Test(Exp_Basic):
    def __init__(self, args):
        ...
        if "ETTh1" in data_path: selected_channels = [2,5,7]
        # elif "ETTh2" in data_path: selected_channels = [1,3,7]
        elif "ETTh2" in data_path: selected_channels = [5,6]
        # elif "ETTm1" in data_path: selected_channels = [1,3, 2,4,5]
        # elif "ETTm1" in data_path: selected_channels = [1,3]
        # elif "ETTm1" in data_path: selected_channels = [2,4,5]
        elif "ETTm1" in data_path: selected_channels = [6,7]
        elif "ETTm2" in data_path: selected_channels = [1,7]  # [1,7, 3]
    ...
    def _get_data(self, flag, batch=None):
        data_set, data_loader = data_provider(self.args, flag, batch=batch)
    ...
    def divide_data_by_period(self, data, data_loader):
        train_xs, train_ys, train_x_marks, train_y_marks = [], [], [], []
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(data_loader):
            train_xs.append(batch_x)
            train_ys.append(batch_y)
            train_x_marks.append(batch_x_mark)
            train_y_marks.append(batch_y_mark)

        train_xs = torch.stack(train_xs, dim=0)
        train_ys = torch.stack(train_ys, dim=0)
        train_x_marks = torch.stack(train_x_marks, dim=0)
        train_y_marks = torch.stack(train_y_marks, dim=0)

        return train_xs, train_ys, train_x_marks, train_y_marks

    
    def select_from_train(self, setting, test=0, is_training_part_params=True, use_adapted_model=True, test_train_epochs=1, weights_given=None, adapted_degree="small", weights_from="test"):
        test_data, test_loader = self._get_data_at_test_time(flag='test')
        if "ETTh1" in self.args.data_path: period = 24
        elif "traffic" in self.args.data_path: period = 24
        elif "national_illness" in self.args.data_path: period = 52
        else:
            raise Exception("this dataset does not exist!!!")
        train_data, train_loader = self._get_data(flag="train_without_shuffle", batch=period)
        ...
        train_xs, train_ys, train_x_marks, train_y_marks = self.divide_data_by_period(train_data, train_loader)
        print(train_xs.shape, train_ys.shape, train_x_marks.shape, train_y_marks.shape)

        train_xs = train_xs.permute(1,0,2,3)
        train_ys = train_ys.permute(1,0,2,3)
        train_x_marks = train_x_marks.permute(1,0,2,3)
        train_y_marks = train_y_marks.permute(1,0,2,3)
        print(train_xs.shape, train_ys.shape, train_x_marks.shape, train_y_marks.shape)
        ...
        PERIOD_DIFF = 0
        selected_train_xs = train_xs[PERIOD_DIFF]
        selected_train_ys = train_ys[PERIOD_DIFF]
        selected_train_x_marks = train_x_marks[PERIOD_DIFF]
        selected_train_y_marks = train_y_marks[PERIOD_DIFF]
        # self.model.eval()
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
            ...
            # 先用原模型的预测值和标签值之间的error，做反向传播之后得到的梯度值gradient_0，并将这个gradient_0作为标准答案
            # 然后，对测试样本做了adaptation之后，会得到一个gradient_1
            # 那么对grad_1和grad_0之间做对比，就可以得到二者之间的余弦值是多少（方向是否一致），以及长度上相差的距离有多少等等。
            if use_adapted_model:
                seq_len = self.args.seq_len
                pred_len = self.args.pred_len
                adapt_start_pos = self.args.adapt_start_pos
            ...
            if i == 0:
                if 'ETTh1' in self.args.data_path:
                    TIME_POS = 0
                    eps = 1e-6
                    cur_hour = batch_y_mark[0][self.args.label_len][TIME_POS]  # get the hour embedding
                    for hour in range(train_y_marks.shape[0]):
                        assert train_y_marks.shape[0] == period
                        ref_hour = train_y_marks[hour][0][self.args.label_len][TIME_POS]
                        if ref_hour - cur_hour < eps:
                            hour_diff = hour
                            break
                else:
                    raise Exception("this dataset does not exist!!!")
                
                PERIOD_DIFF = hour_diff
                print("PERIOD_DIFF is", PERIOD_DIFF)
                selected_train_xs = train_xs[PERIOD_DIFF]
                selected_train_ys = train_ys[PERIOD_DIFF]
                selected_train_x_marks = train_x_marks[PERIOD_DIFF]
                selected_train_y_marks = train_y_marks[PERIOD_DIFF]
                print(selected_train_xs.shape, selected_train_ys.shape, selected_train_x_marks.shape, selected_train_y_marks.shape)            
            ...
            for epoch in range(test_train_epochs):
                ...
                import random
                # is_random = True
                is_random = True if self.args.random_select else False
                sample_indices = list(range(selected_train_xs.shape[0]))
                if is_random:  # 随机取出test_train_num个
                    sample_indices = random.sample(sample_indices, self.test_train_num)
                else:  # 取出最后的test_train_num个
                    sample_indices = sample_indices[-self.test_train_num:]
                ...
                for ii in sample_indices:
                    ...
                    pred, true = self._process_one_batch_with_model(cur_model, test_data,
                        selected_train_xs[ii:ii+1], selected_train_ys[ii:ii+1], 
                        selected_train_x_marks[ii:ii+1], selected_train_y_marks[ii:ii+1])
            ...
            if (i+1) % 100 == 0:
                ...
                print(f"last_sample_indices are: {sample_indices}")

                printed_selected_channels = [item+1 for item in self.selected_channels]
                printed_unselected_channels = [item+1 for item in unselected_channels]
                print(f"selected_channels: {printed_selected_channels}")
                print(f"unselected_channels: {printed_unselected_channels}")
