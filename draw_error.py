    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            if args.run_test:
                ...
                loss_1 = exp.test(setting, test=1, flag="train_without_shuffle")
                loss_2 = exp.test(setting, test=1, flag="val_without_shuffle")
                loss_3 = exp.test(setting, test=1, flag="test")
                print(len(loss_1), len(loss_2), len(loss_3))
                # print(loss_1)
                
                dir = "./error_by_time"
                def write_error(phase, loss_list):
                    with open(f"{dir}/{args.figure_name}_{phase}.txt", "w") as f:
                        for item in loss_list:
                            f.write(str(item))
                            f.write("\n")
                write_error("train", loss_1)
                write_error("vali", loss_2)
                write_error("test", loss_3)
                ...
                # x_1 = [i for i in range(len(loss_1))]
                # x_2 = [i for i in range(len(loss_1), len(loss_1)+len(loss_2))]
                # x_3 = [i for i in range(len(loss_1)+len(loss_2), len(loss_1)+len(loss_2)+len(loss_3))]

                # import matplotlib.pyplot as plt
                # plt.title("loss function")
                # plt.xlabel("running times")
                # plt.ylabel("loss value")
                # plt.plot(x_1, loss_1, label="loss", color='b')
                # plt.plot(x_2, loss_2, label="loss", color='r')
                # plt.plot(x_3, loss_3, label="loss", color='g')
                # plt.savefig("./tmp_figures/error_with_time_Weather_batch1.png")
            ...
            if args.run_adapt:
                ...
                a1, a2, a3 = exp.my_test(setting, test=1, is_training_part_params=True, use_adapted_model=True, test_train_epochs=args.test_train_epochs)
                ...
                def write_error(phase, loss_list):
                    with open(f"{dir}/{args.figure_name}_{phase}.txt", "w") as f:
                        for item in loss_list:
                            f.write(str(item))
                            f.write("\n")
                write_error("before_adapt", a1)
                write_error("adapting", a2)
                write_error("after_adapt", a3)
